...
async function insert(table, data) {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, {
    method: "POST",
    headers: { "apikey": ANON_KEY, "Authorization": `Bearer ${SERVICE_ROLE_KEY}`, "Content-Type": "application/json", "Prefer": "return=representation" },
    body: JSON.stringify(data)
  });
  if (!response.ok) throw new Error(`Error al insertar en ${table}: ${await response.text()}`);
...
async function select(table) {
  ...
  if (!response.ok) throw new Error(`Error al consultar ${table}: ${await response.text()}`);
  return response.json();
}
...
async function update(table, id, data) {
  ...
  if (!response.ok) throw new Error(`Error al actualizar en ${table}: ${await response.text()}`);
  return response.json();
}
...
async function remove(table, id) {
  ...
  if (response.status !== 204) throw new Error(`Error al eliminar de ${table} (status ${response.status}): ${await response.text()}`);
  return { success: true };
}
...
async function main() {
  try {
    ...
    console.log("Asumiendo que las tablas 'usuarios', 'productos' y 'pedidos' ya existen.");
    console.log("\n--- Demostración de uso de la API ---");
    console.log("Insertando un nuevo usuario...");
    const nuevoUsuario = await insert("usuarios", { nombre: "Astaroth", correo: `astaroth-${Date.now()}@infernal.dev` });
    ...
    console.log(`Usuarios encontrados (${todosLosUsuarios.length}):`, todosLosUsuarios);
    console.log(`\nActualizando el nombre del usuario con ID ${usuarioId}...
`);
    const usuarioActualizado = await update("usuarios", usuarioId, { nombre: "Astaroth, El Gran Duque" });
...
`);
    ...
    console.error("\n❌ Error durante la ejecución de la prueba:", error.message);
  }
}
