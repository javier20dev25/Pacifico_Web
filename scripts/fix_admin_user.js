...
async function fixAdminUser() {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      console.error(
        `❌ Error: No se encontró ningún usuario con el correo '${ADMIN_EMAIL}'.`
      );
      console.log(
        'Por favor, asegúrate de que el usuario exista en la tabla "usuarios".'
      );
      return;
    }

    console.log(
      '✅ ¡Éxito! El usuario administrador ha sido actualizado correctamente.'
    );
