...
import useStore from '@/stores/store';// Mock del apiClient (axios)
...
vi.mock('@/stores/store', () => {
  ...
  const mockStoreData = {
    ...
    return state;
  };  mockUseStore.getState = () => ({ ...mockStoreData, setStoreDetails });  return {
    __esModule: true,
  ...
  beforeEach(() => {
    ...
    const { setStoreDetails: setStoreDetailsMock } = useStore.getState();    // 1. Clic en el botón "Editar"
    fireEvent.click(screen.getByRole('button', { name: /editar/i }));    // 2. Cambiar el nombre
    ...
    fireEvent.click(screen.getByRole('button', { name: /^guardar$/i }));    // 4. Verificar que la acción del store fue llamada
