...
import RegistrationChart from '@/components/admin/RegistrationChart';const AdminDashboard = () => {
  const [users, setUsers] = useState([]);
  const [stats, setStats] = useState({ temp: 0, active: 0 });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);  const fetchUsers = async () => {
    // Envolvemos la lógica en una función que podamos reutilizar
    try {
      setLoading(true);
      const response = await apiClient.get('/admin/users');
      const fetchedUsers = response.data || [];
      setUsers(fetchedUsers);      let tempCount = 0;
      let activeCount = 0;
      fetchedUsers.forEach(user => {
        if (user.status === 'temporary') tempCount++;
        if (user.status === 'active') activeCount++;
      });
      setStats({ temp: tempCount, active: activeCount });    } catch (err) {
