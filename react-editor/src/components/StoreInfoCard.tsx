  ...
  return (
    <div className="p-4 border rounded-md bg-white shadow-sm">
      <div className="flex justify-between items-center mb-4">
        ...
        <input type="file" id="logo-upload" accept="image/*" className="hidden" onChange={handleLogoChange} />        <label htmlFor="nombre" className="block text-sm font-medium text-gray-500">Nombre de la tienda</label>
        <input id="nombre" type="text" className="input w-full p-2 border rounded" 
               value={store.nombre || ''} onChange={handleInputChange} />        <label htmlFor="descripcion" className="block text-sm font-medium text-gray-500">Descripción</label>
        <textarea id="descripcion" className="input w-full p-2 border rounded resize-y"
                  value={store.descripcion || ''} onChange={handleInputChange}></textarea>        <label htmlFor="whatsapp" className="block text-sm font-medium text-gray-500">WhatsApp</label>
