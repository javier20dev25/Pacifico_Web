...
const StoreManager: React.FC<StoreManagerProps> = ({ stores }) => {
  ...
  if (!stores || stores.length === 0) {
    return (
      <div className="bg-white shadow-md rounded-lg p-8 text-center mb-8"> {/* Added mb-8 for consistent spacing */}
                  <h2 className="text-2xl font-bold text-googleBlue mb-4">Mi Tienda</h2>
                  <p className="text-neutral-600 mb-6">Aún no has creado tu tienda.</p>
                            <Link to="/editor" className="inline-flex items-center bg-googleBlue text-white font-semibold py-3 px-6 rounded-lg shadow-md hover:bg-googleBlue transition duration-300">
  ...
  return (
    <div className="mb-8"> {/* Added mb-8 for consistent spacing */}
          <h2 className="text-2xl font-bold text-googleBlue mb-6">{storeName}</h2>
          <div className="bg-googleBlue-50 shadow-lg rounded-lg border-t-4 border-googleBlue p-6">
            <div className="aspect-w-16 aspect-h-9 border-2 border-googleBlue rounded-lg overflow-hidden mb-4 bg-neutral-100">
              {publicUrl !== '#' ? (
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              {isLaunched ? (
                ...
                <Link to="/editor" className="flex-1 sm:flex-none inline-flex items-center justify-center text-center bg-googleBlue text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-googleBlue transition duration-300">
                  <PencilIcon className="h-5 w-5 mr-2" />
                  Editar Tienda
                </Link>
              )}
              <a href={publicUrl} target="_blank" rel="noopener noreferrer" className="flex-1 sm:flex-none inline-flex items-center justify-center text-center bg-googleGreen text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-googleGreen transition duration-300">
                <EyeIcon className="h-5 w-5 mr-2" />
                Ver Tienda
              </a>
              <button onClick={() => handleShare(publicUrl)} className="flex-1 sm:flex-none inline-flex items-center justify-center bg-googleYellow text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-googleYellow transition duration-300">
                <ShareIcon className="h-5 w-5 mr-2" />
                Compartir
              </button>
              <button onClick={handleDelete} className="flex-1 sm:flex-none inline-flex items-center justify-center bg-googleRed text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-googleRed transition duration-300">
