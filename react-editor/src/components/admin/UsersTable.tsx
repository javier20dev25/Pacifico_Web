...
const CredentialsModal = ({ credentials, isOpen, onClose, onCopy }) => {
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(message)
      .then(() => {
        ...
        onCopy(); // Opcional: para cualquier lógica extra después de copiar
      })
...
const UsersTable = ({ users, onUserAction }) => {
  ...
  const handleShowCredentials = async (userUuid) => {
    setIsLoadingCredentials(true);
    try {
      const response = await apiClient.get(`/admin/credentials/${userUuid}`);
      setCurrentCredentials(response.data);
      setModalOpen(true);
    } catch (err) {
      alert(err.response?.data?.error || 'Error al obtener las credenciales.');
    } finally {
  ...
  const handleAction = async (action, user) => {
    // Interceptar la acción de mostrar credenciales
    ...
    if (window.confirm(actionConfig.confirm)) {
      try {
        ...
        const response = await apiClient.post('/admin' + actionConfig.endpoint, payload);
