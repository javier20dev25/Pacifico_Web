...
const protect = async (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET); // Adjuntar el usuario al objeto de la petición para uso posterior
    req.user = decoded;
    next();
  } catch (error) {
...
const isAdmin = (req, res, next) => {
  try {
    if (!req.user) return res.status(401).json({ error: 'No autenticado.' }); // 1. Comprobación de Super Administrador por email (desde .env)
