...
router.post('/create-temporary-user', async (req, res) => {
  ...
  try {
    ...
    if (!nombre || !plan_nombre) {
      return res
        .status(400)
        .json({ error: 'Nombre de tienda y plan son obligatorios.' });
    }
    ...
    if (correoEnviado && correoEnviado.trim()) {
      ...
      if (existingError && existingError.code !== 'PGRST116') {
        console.error(
          'Supabase error checking correo (admin provided):',
          existingError
        );
        return res
          .status(500)
          .json({ error: 'Ocurrió un error al verificar el correo.' });
      }
    } else {
      ...
      while (true) {
        ...
        if (existingError && existingError.code !== 'PGRST116') {
          console.error(
            'Supabase error checking correo (generated):',
            existingError
          );
          return res
            .status(500)
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        return res.status(409).json({
          error:
            'El correo electrónico ya está en uso (violación de unicidad).',
        });
      }
      return res
        .status(500)
        .json({ error: 'Ocurrió un error al crear el usuario y el contrato.' });
    }
