async function checkTable() {
    ...
    try {
        const response = await fetch(`${SUPABASE_URL}/rest/v1/pedidos?select=*&limit=1`, {
            headers: {
                "apikey": ANON_KEY,
        ...
        if (response.ok) {
            ...
            if (error.code === '42P01') { 
                console.error("❌ Error: La tabla 'pedidos' no existe. Por favor, créala en tu panel de Supabase.");
            } else {
                console.error(`❌ Error inesperado al consultar la tabla. Código: ${response.status}`, error);
            }
        }
    } catch (e) {
